...
{
    public partial class Financial_Analysis : PageBase
    {
        ...
        private void DataBindGrid(bool isQuery = false)
        {
            ...
            dt = ordersManager.FinanceReport(startDate, endDate);
        ...
        protected void lbtExport_Click(object sender, EventArgs e)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //写出列名
                s.AppendLine("<tr>");
                s.AppendLine("<td>日期</td><td>订单支付金额</td><td>充值金额</td><td>押金缴纳金额</td><td>押金退款金额</td>");
                s.AppendLine("</tr>");
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    s.Append("<td>").Append(row["OrderPayMoney"].ToString()).Append("</td>");
                    s.Append("<td>").Append(row["ChangesAmount"].ToString()).Append("</td>");
                    s.Append("<td>").Append(row["DepositMoney"].ToString()).Append("</td>");
                    s.Append("<td>").Append(row["ReturnMoney"].ToString()).Append("</td>");
                    s.AppendLine("</tr>");
                }
                s.AppendLine("</table>");
                ...
                this.Page.Response.AppendHeader("Content-Disposition", "attachment;filename=小宝出行财务分析" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls");
