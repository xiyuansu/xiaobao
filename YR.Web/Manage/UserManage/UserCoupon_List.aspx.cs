...
{
    public partial class UserCoupon_List : PageBase
    {
        ...
        private void DataBindGrid(bool isQuery = false)
        {
            ...
            DataTable dt = couponManager.GetPage(keyValue.Key, keyValue.Value, pageIndex, PageControl1.PageSize, ref count);
        ...
        private KeyValuePair<StringBuilder, IList<SqlParam>> InitCondition()
        {
            ...
            {
                sb.Append(" and RealName like @RealName or NickName like @UserName");
                IList_param.Add(new SqlParam("@RealName", '%' + txtName.Value.Trim() + '%'));
            }
            if (!string.IsNullOrEmpty(txtTel.Value))
            {
                sb.Append(" and BindPhone like @BindPhone ");
