...
namespace YR.Web.api.app.pay.alipay
{
    ...
    public class Core
    {
        ...
        public static string GetOrderInfo(string out_trade_no, string subject, string body, string notifyurl, decimal paymoney)
        {
            try
            {
                StringBuilder biz_content=new StringBuilder();
                biz_content.Append(@"{");
                biz_content.AppendFormat("\"timeout_express\":\"{0}\",","90m");
                biz_content.AppendFormat("\"product_code\":\"{0}\",","QUICK_MSECURITY_PAY");
                biz_content.AppendFormat("\"total_amount\":\"{0}\",",paymoney.ToString("N2"));
                biz_content.AppendFormat("\"subject\":\"{0}\",",subject);
                biz_content.AppendFormat("\"body\":\"{0}\",",body);
                ...
                dicArray.Add("charset", "utf-8");
                dicArray.Add("format", "JSON");
                dicArray.Add("method", "alipay.trade.app.pay");
                dicArray.Add("notify_url", notifyurl);
                dicArray.Add("sign_type", sign_type);
                dicArray.Add("timestamp", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                dicArray.Add("version", "1.0");
                string orderinfo = CreateLinkString(dicArray);
                string sign= RSA.sign(orderinfo, Config.private_key, "UTF-8", sign_type);
                
                dicArray.Add("sign", sign);
                return CreateLinkStringUrlencode(dicArray,Encoding.UTF8);
            }
            catch(Exception e)
            {
                return "";
