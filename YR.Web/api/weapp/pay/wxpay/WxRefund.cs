    ...
    public class WxRefund
    {
        ...
        public bool refund(string out_trade_no, decimal refund_amount, out string errmsg)
        {
            ...
            try
            {
                ...
                dic.Add("appid", WxConfig.AppId);
                dic.Add("mch_id", WxConfig.MchId);
                ...
                dic.Add("op_user_id", WxConfig.MchId);
                ...
                Logger.Error(string.Format("微信押金退款{0}({1}):{2}", result ? "成功" : "失败", out_trade_no, errmsg));
                return result;
            }
            catch (Exception ex)
            {
                Logger.Error(string.Format("微信押金退款失败({0}):{1}", out_trade_no, ex.Message));
                return false;
            }
        }
        ...
        public bool refund2(string transationid, decimal refund_amount,out string errmsg)
        {
            ...
            try
            {
                ...
                Logger.Error(string.Format("微信押金退款{0}({1}):{2}", result ? "成功" : "失败", transationid, errmsg));
                return result;
            }
            catch (Exception ex)
            {
                Logger.Error(string.Format("微信押金退款失败({0}):{1}", transationid, ex.Message));
                return false;
            }
        }
        ...
        private string HttpsPost(string url, string postDataStr, string codename)
        {
            ...
            cert = SiteHelper.GetAppsetString("weapp_api_cert");
